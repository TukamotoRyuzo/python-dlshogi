...
def MyPolicyValueResnet(blocks):
    ...
    x = layers.Conv2D(
        ch, 3, padding='same', kernel_initializer='he_normal',
        name="init_conv")(board_image)
    ...
    # policy network
    x = layers.Conv2D(
        MOVE_DIRECTION_LABEL_NUM, 1, padding='same',
    ...
    policy_value_model = Model(
        inputs=board_image, outputs=[policy_out, value_out])
    ...
    def __init__(self):
        ...
        with self.init_scope():
            self.conv1 = L.Convolution2D(
                in_channels=ch, out_channels=ch, ksize=3, pad=1, nobias=True)
...
class PolicyValueResnet(Chain):
    ...
    def __init__(self, blocks=5):
        ...
        with self.init_scope():
            self.l1 = L.Convolution2D(
                in_channels=104, out_channels=ch, ksize=3, pad=1)
    ...
    def __call__(self, x):
        ...
        u_policy = self.policy_bias(
            F.reshape(h_policy, (-1, 9 * 9 * MOVE_DIRECTION_LABEL_NUM)))
        # value network
